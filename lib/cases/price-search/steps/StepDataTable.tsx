...
interface PriceDataRow extends BaseRow {
  product: string
  price: string
...
export function StepDataTable({
  ...
  const handleFillData = React.useCallback(() => {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.debug("[price-search:data-table] fill start", { totalRows })
    }
  }, [isFilling, totalRows])
...
export function toPriceRow(item: string, index: number): PriceDataRow {
  return {
    id: index + 1,
    product: item,
    price: "",
