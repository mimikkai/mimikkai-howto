...
interface ThreadsDataRow extends BaseRow {
  date: string
  postUrl: string
  postText: string
  comment: string
...
export function StepDataTable({
  ...
  const caseConfig = threadsCommentsConfig
  ...
  const dataLength = threadsData.length
  ...
  const handleFillData = React.useCallback(() => {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.debug("[threads-comments:data-table] fill start", { totalRows })
    }
  }, [isFilling, totalRows])
  ...
  }, [threadsData, isFilling])

  const columns = caseConfig.dataTable.columns

  const getColumnValue = (row: ThreadsDataRow, colId: string): string => {
    switch (colId) {
...
export function toThreadsRow(item: { date: string }, index: number): ThreadsDataRow {
  return {
    id: index + 1,
    date: item.date,
    postUrl: "",
