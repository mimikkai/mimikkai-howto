...
interface TicketRow extends BaseRow {
  date: string
  question: string
  answer: string
  source: string
...
export function StepDataTable({
  ...
  const caseConfig = ticketReplyConfig
  const [ticketData, setTicketData] = React.useState<TicketRow[]>([])
  ...
  const dataLength = ticketData.length
  ...
  const handleFillData = React.useCallback(() => {
    ...
    const interval = setInterval(() => {
      ...
      for (let i = start; i < end; i++) {
        newRows.push({
          id: i + 1,
          date: TICKETS[i].date,
          question: TICKETS[i].question,
          answer: "",
          source: "",
          status: "ожидает" as const,
        })
      }
      setTicketData((prev) => [...prev, ...newRows])
    ...
    if (process.env.NODE_ENV !== "production") {
      console.debug("[ticket-reply:data-table] fill start", { totalRows })
    }
  }, [isFilling, totalRows])
              ...
              <table className="w-full text-xs">
                <thead className="sticky top-0 z-10 bg-card shadow-[0_1px_0_var(--color-border)]">
                  <tr className="border-b text-left text-muted-foreground">
                    ...
                    <th className="w-28 px-3 py-2 font-medium">D — Статус</th>
                  </tr>
                </thead>
                <tbody>
                  {ticketData.map((row, index) => {
                    const isNew =
                      index >= ticketData.length - lastAddedCount && isFilling
                    return (
                      <tr
                        key={row.id}
                      ...
                      >
                        ...
                        <td className="max-w-[200px] truncate px-3 py-2 font-medium">{row.question}</td>
                        <td className="hidden px-3 py-2 text-muted-foreground sm:table-cell">
                          {row.answer || "—"}
                        </td>
                        <td className="px-3 py-2">
                          <span
                            className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${ticketStatusColor[row.status]}`}
                          >
                            {row.status}
                          </span>
                        </td>
...
export function toTicketRow(ticket: { date: string; question: string }, index: number): TicketRow {
  return {
    id: index + 1,
    date: ticket.date,
