...
export function StepChat({ mim, onNext, onBack }: StepChatProps) {
  ...
  const ASSISTANT_GREETING = "Привет! Я помогу создать ИИ-агента на платформе MimikkAi.\n\nОпишите, какого агента вы хотите создать:"
  const PRESET_PROMPT = mim.name
  const PRESET_PROMPT_DESC = mim.description.split("\n").slice(0, 2).join(" ")
  const ASSISTANT_RESPONSE = mim.description
  ...
  React.useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      console.debug("[playground:step-chat] render", {
        mimName: mim.name,
        chatVisible,
        isDone,
      })
    }
  }, [mim.name, chatVisible, isDone])
